...
@click.option("--max-iterations", default=50, type=int, help="Max iterations per scanner swarm")
...
async def _run_all_targets(
    ...
    await _write_report(all_findings, targets[0], total_tracker, output, json_output)
...
async def _write_report(findings, target, tracker, output: str, json_output: str) -> None:
